function getSearchProductLists(event) {
    ...
    let rooturl = `http://localhost:8131/product?id=${id}`;
    if(!id) rooturl = `http://localhost:8131/products`;
    console.log(id);
        ...
        if (data.data.length === 0) {
            ...
            let tableHTML = `
                ...
                data.data.forEach(product => {
                    tableHTML += `
                        <tr  onclick="window.location.href='../../sec3_gr7_fe_src/Preview_Product_page/Preview_Product_Page.html?productID=${product.ProductID}'">
...
function insertNewProduct(event) {
    ...
    let rooturl = `http://localhost:8131/postproduct`; // Encode query parameter
    console.log(rooturl);
...
function updateProduct(event){
    ...
    let rooturl = `http://localhost:8131/updateproduct?ProductID=${id}`;
...
function deleteProduct(event){
    ...
    let rooturl = `http://localhost:8131/deleteproduct/id=${id}`;
