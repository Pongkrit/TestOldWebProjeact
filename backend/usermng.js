function getSearchAccountLists(event) {
    ...
    let rooturl = `http://localhost:8131/account?id=${id}`;
    if(!id) rooturl = `http://localhost:8131/accounts`;
    console.log(id);
        ...
        if (data.data.length === 0) {
            ...
            let tableHTML = `
                ...
                data.data.forEach(account => {
                    tableHTML += `
                        <tr  onclick="window.location.href='../../sec3_gr7_fe_src/Admin_Profile1/adminprofile1.html?EmpID=${account.EmpID}'">
                            <td>${account.EmpID}</td>
...
function insertNewUser(event) {
    ...
    let rooturl = `http://localhost:8131/postaccount`; // Encode query parameter
    console.log(rooturl);
...
function updateUser(event){
    ...
    let rooturl = `http://localhost:8131/updateaccount?EmpID=${id}`;
...
function deleteUser(event){
    ...
    let rooturl = `http://localhost:8131/deleteaccount/id=${id}`;
